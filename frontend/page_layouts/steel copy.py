...
def sample_data():
    data = {
        "total_spend": 485000.0,
        "total_paid": 420000.0,
        "outstanding_amount": 65000.0,
        "avg_cost_per_kg": 62.50,
        "binding_wire": {
            "bundles": 6,
            "cost": 12000.0
        },
        "steel_size_breakdown": {
            "TMT 8mm": {"bundles": 10, "weight": 452.5, "cost": 125000.0},
            "TMT 10mm": {"bundles": 12, "weight": 587.3, "cost": 158000.0},
            "TMT 12mm": {"bundles": 8, "weight": 412.0, "cost": 148000.0},
            "TMT 16mm": {"bundles": 5, "weight": 275.5, "cost": 42000.0}
        }
    }

def render_steel_metrics():
    ...
    if wire.get("bundles", 0) > 0:
        st.caption("🔗 Binding Wire")
        wire_metrics = {
            "Bundles": f"{wire['bundles']}",
            "Total Cost": f"₹ {wire['cost']:,.0f}"
        }
        render_data_metrics_style3(dict_datametrics=wire_metrics)

    # Row 3: Steel by size
    size_breakdown = data.get("steel_size_breakdown", {})
    if size_breakdown:
        st.caption("🏗️ Steel (by Size)")
        size_metrics = {}
        for size, info in size_breakdown.items():
            size_metrics[f"{size} Bundles"] = f"{info['bundles']}"
            size_metrics[f"{size} Weight"] = f"{info['weight']:,.1f} KG"
